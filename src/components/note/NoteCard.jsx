...
function NoteCard({ note, active = false, onClick, onTrash, onDuplicate, onTogglePin }) {
    ...
    <div className="relative">
      <button
        className={`w-full rounded-xl border p-4 pr-28 text-left transition hover:scale-[1.01] ${
          active ? 'border-indigo-400' : 'border-slate-800'
        } ${colorMap[note.color] || colorMap.slate}`}
        onClick={onClick}
      >
        <div className="mb-2 flex items-center justify-between gap-2">
          <h3 className="line-clamp-1 font-medium text-slate-100">{note.title || 'Basliksiz'}</h3>
          {note.pinned ? <Pin size={16} className="text-amber-300" /> : null}
        </div>
        <p className="line-clamp-2 min-h-10 text-sm text-slate-300">{previewText || 'Henuz icerik yok...'}</p>
        ...
        <p className="mt-3 text-xs text-slate-400">
          Son guncelleme: {new Date(note.lastEditedAt || note.updatedAt).toLocaleString()}
        </p>
      </button>
